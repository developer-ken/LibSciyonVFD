...
namespace LibSciyonVFD
{
    public partial class VFDConfiguration
    {
		...
		private Dictionary<string, object> _aliasByCode;
		public Dictionary<string, object> AliasByCode => _aliasByCode ?? (_aliasByCode = BuildAliasDictionary());

		private Dictionary<string, object> BuildAliasDictionary()
		{
		...
		public void Deserialize(string path)
		{
			...
			foreach (var raw in File.ReadAllLines(path, Encoding.UTF8))
			{
				...
				if (!AliasByCode.TryGetValue(fullKey, out var item)) continue;
				var fi = item.GetType().GetField("Value");
				if (fi == null) continue;
